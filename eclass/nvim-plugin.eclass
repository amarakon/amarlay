...
# @DESCRIPTION:
# This eclass simplifies installation of app-nvim plugins into
# /usr/share/nvim/runtime.  This is a version-independent directory
...
#DEPEND="|| ( >=app-editors/neovim-${VIM_PLUGIN_VIM_VERSION}
#	>=app-editors/neovim-qt-${VIM_PLUGIN_VIM_VERSION} )"
#RDEPEND="${DEPEND}"
#if [[ ${PV} != 9999* ]] ; then
...
# * installs all files in "${ED}"/usr/share/nvim/runtime.
...
# * update_nvim_afterscripts
# * display_nvim_plugin_help
nvim-plugin_pkg_postinst() {
	update_nvim_helptags		# from vim-doc
	update_nvim_afterscripts	# see below
	display_nvim_plugin_help	# see below
}
...
nvim-plugin_pkg_postrm() {
	...
	#find "${EPREFIX}/usr/share/nvim/runtime" -depth -type d -exec rmdir {} \; 2>/dev/null || \
		#die "rmdir failed"
}

# @FUNCTION: update_nvim_afterscripts
# @DESCRIPTION:
# Creates scripts in /usr/share/nvim/runtime/after/*
# comprised of the snippets in /usr/share/nvim/runtime/after/*/*.d
update_nvim_afterscripts() {
	has "${EAPI:-0}" 0 1 2 && ! use prefix && EROOT="${ROOT}"
	has "${EAPI:-0}" 0 1 2 && ! use prefix && EPREFIX=
	local d f afterdir="${EROOT}"/usr/share/nvim/runtime/after

	# Nothing to do if the dir isn't there
	[ -d "${afterdir}" ] || return 0

	einfo "Updating scripts in ${EPREFIX}/usr/share/nvim/runtime/after"
	find "${afterdir}" -type d -name \*.vim.d | while read d; do
		echo '" Generated by update_nvim_afterscripts' > "${d%.d}" || die
		find "${d}" -name \*.vim -type f -maxdepth 1 -print0 | sort -z | \
			xargs -0 cat >> "${d%.d}" || die "update_vim_afterscripts failed"
	done

	einfo "Removing dead scripts in ${EPREFIX}/usr/share/nvim/runtime/after"
	find "${afterdir}" -type f -name \*.vim | \
	while read f; do
		[[ "$(head -n 1 ${f})" == '" Generated by update_nvim_afterscripts' ]] \
			|| continue
...
# @FUNCTION: display_nvim_plugin_help
# @DESCRIPTION:
...
display_nvim_plugin_help() {
	...
	if [[ -z ${REPLACING_VERSIONS} ]]; then
		...
		if has filetype ${VIM_PLUGIN_MESSAGES}; then
			...
			elog "to your ~/.config/nvim/init.vim file."
			elog " "
		fi
	fi
}
